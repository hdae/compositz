...
| **2 — Management UI**                                                                    | 🔄 In progress — `packages/ui` (Fresh 2/Vite): instance list + live status + up/down + install build-log + **RI-2 tar/tar.gz/dir ingestion** done; **RI-3 GitHub sourcing done in core+CLI** (ADR-021). Next: RI-3 UI entry, RI-4 override UI. Hono retired (ADR-013) |
