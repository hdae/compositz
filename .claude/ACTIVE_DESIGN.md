...
bindings 生成）✅ / 4b（runnable 縦切り実装）✅ / 4c（dialogs: import/trust/delete/duplicate）✅
（0abc2ff+abc7bbc・静的緑・実ブラウザ 28/28 ×4 安定）。次は 4d（detail タブ）。** 計画:
...
**NEXT: 4d〜4f — React UI 残り。** 4d detail タブ(build/runtime log=stream_logs・services・settings=
get_config/set_config・restartNeeded)、4e banners/drag-drop/open/export(exportMount+save picker)、4f
parity 総点検 + scaffold 残骸掃除(frontend/README.md・AGENTS.md・`dependencies` の `shadcn`)。挙動 parity は
各 ADR(execution 計画 Phase 4 節・InstanceList.tsx が仕様)。**★user 優先(07-04): runnable 優先・テスト/CI
