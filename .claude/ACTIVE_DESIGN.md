...
**D-cli 実装済み + ★Windows 実機確認 ✅（2026-07-14 全7項目クリア）**:
`wslc_cli.rs`（同一 ContainerCreateBody から argv 射影・env は --env-file・
表現不能 spec は fail loud）/ `start_with` の endpoint 分岐 / 全 listing を
`list_raw_summaries` に集約して label から VmPort→HostPort 翻訳。実機確認 =
バッジ wslc·online / install→Start→**ブラウザ localhost 到達** / `wslc container
list` に `127.0.0.1:8090->80/tcp` 表示 / ready 遷移+Open / Stop で消滅+不達 /
**アプリ終了後も relay 存続（wslc サービス持ち = Docker Desktop パリティ）**。
wslc アークの残余（急がない）: daemon 自動起動の明示確認・長時間ストリーム・
GPU ホストでの --gpus 委譲・endpoint 自動検出（engine 接続設定アークと同時）。
**upstream 報告は要検討のまま保留 — wslc GA 後に再訪（user 決定 2026-07-14）。**
**NEXT（user 選択済み 2026-07-14）: Compositz 自体の設定 UI アーク** — 中核は
roadmap「Engine connection settings」（endpoint を env 変数から UI 設定+永続化へ:
Docker Desktop / rootless / remote TCP / wslc、バッジは実装済み）。設定の置き場・
適用タイミング（再接続）・既存 `COMPOSITZ_DOCKER_HOST` との優先順位などを含め
**recon→計画→承認から開始**（まだ何も設計していない）。
保留中: `docs/plans/slice-c-build-args.md` / `docs/plans/gc-disk-usage.md`
...
実機事前チェック①②③ 全通過（named volume ✓ / local image no-pull ✓ /
create+start relay 到達 + 外部 stop で list から消え port 不達 ✓）→ **D-cli 実装完了**:
931b72c（wslc_cli argv 射影+env-file+stub spawn テスト）/ 3d1eec8（start_with 分岐）/
e3f2469（list 層 VmPort→HostPort 翻訳）/ +docs（ADR-031・limitations・roadmap）。
検証 = core 全テスト緑（新規: 射影8 + 単体11）/ clippy 0 / desktop flake 緑 /
bindings 鮮度 ✓ / E2E 実 engine 緑（9.13s, bollard 経路無傷）。

**★Windows 実機確認 ✅（2026-07-14, 全7項目）**: バッジ / install→Start→ブラウザ
到達 / wslc list に `127.0.0.1:8090->80/tcp` / ready+Open / Stop で消滅+不達 /
アプリ終了後の relay 存続。**wslc アーク（transport+port relay）はこのスライスで完結。**
upstream 報告 = 保留（GA 後再訪）。

**compact 後の再開手順: 「Compositz 設定 UI アーク」の recon から**（上の NEXT 参照。
対象コードの現状 = endpoint は `COMPOSITZ_DOCKER_HOST` env のみ・`resolved_endpoint_summary`
がバッジ用 kind/description を導出・接続は起動時 `connect()` 一回。設計論点の種:
設定の永続化場所 / env との優先順位 / 再接続の適用タイミング / doctor・バッジ連動 /
wslc を選択肢としてどう見せるか）。コード変更は承認後。
