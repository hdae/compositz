...
**D-cli 実装済み（ADR-031, 事前チェック①②③全通過後に着手・全ゲート緑）**:
`wslc_cli.rs`（同一 ContainerCreateBody から argv 射影・env は --env-file・
表現不能 spec は fail loud）/ `start_with` の endpoint 分岐 / 全 listing を
`list_raw_summaries` に集約して label から VmPort→HostPort 翻訳。
**NEXT: Windows 実機確認（wslc で import→install→up→ブラウザ→down、
`wslc container list` 表示、アプリ終了後の relay 存続）。**
...
**★次の入力待ち = Windows 実機確認**: wslc endpoint で Hello Web を
import→install→up → ①バッジ wslc·online ②`wslc container list` に compositz-* が
表示（ポート列は 127.0.0.1:xxxx） ③ブラウザで localhost:port 到達 ④UI が ready 遷移
+ Services の Open 動作 ⑤down で list から消え port 不達 ⑥up し直し→アプリ終了→
ブラウザ到達維持（relay が wslc サービス持ちの確証）。
