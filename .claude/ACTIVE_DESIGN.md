...
**Tauri 全面移行 — Phase 1〜3 完了。Phase 4（React UI）着手中: 4a（flake ビルド環境 + bindings
生成）✅ / 4b（runnable 縦切り）✅ / 4c（dialogs）✅ / 4d（detail タブ: build/runtime log・services・
settings）✅（61941f7+56ee654・実ブラウザ 20/20 + 敵対的レビュー全 holds）。次は 4e（banners/
drag-drop/export）。** 計画:
...
**NEXT: 4e〜4f — React UI 残り。** 4e banners/drag-drop/open/export(exportMount+save picker)、4f parity
総点検 + scaffold 残骸掃除(frontend/README.md・AGENTS.md・`dependencies` の `shadcn`)。挙動 parity は各 ADR
(execution 計画 Phase 4 節・InstanceList.tsx が仕様)。**★user 優先(07-04): runnable 優先・テスト/CI 後回し・
ローカルは vp dev + browser + mockIPC、実 backend は Windows。**
