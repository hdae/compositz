...
Deno.test("toInstanceView: maps identity, name, version, description", () => {
  const view = toInstanceView(instance(baseManifest));
...
Deno.test("toInstanceView: a missing description becomes an empty string", () => {
  const { description: _drop, ...noDesc } = baseManifest;
  const view = toInstanceView(instance(noDesc satisfies Manifest));
  assertEquals(view.description, "");
});

Deno.test("toInstanceView: webPorts includes ONLY web:true ports, with the view fields", () => {
  const manifest = {
    ...baseManifest,
    ports: [
      {
  ...
  const view = toInstanceView(instance(manifest));
  assertEquals(view.webPorts, [
    { name: "ui", container: 8188, protocol: "tcp", path: "/", description: "Web UI." },
    { name: "alt", container: 8189, protocol: "tcp", path: "/admin", description: undefined },
  ]);
});

Deno.test("toInstanceView: imageTag for a build recipe carries the instanceId + version", () => {
  const view = toInstanceView(instance(baseManifest));
  assertStringIncludes(view.imageTag, "hello-a1b2c3d4");
  assertStringIncludes(view.imageTag, "0.1.0");
});

Deno.test("toInstanceView: an image-based recipe uses the external image as the tag", () => {
